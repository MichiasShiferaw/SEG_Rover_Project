...
/**
  * A class representing a NASA Lunar Rover’s mobility implementation. This class has common traits of a Rover such as its current state represented by an enum (a special collection) value as well as another
...
public class Rover{
  ...
  public enum State { 
	  ...
	  vehiculeOn}
  /**Initialize an enum collection that consists of the state a Rover can have turn when the engine is running*/
  ...
  public String getStateFullName(){
	  ...
	  if (stateVehiculeOn != StateVehiculeOn.Null) {
		  answer += "." + stateVehiculeOn.toString(); 
		  }
	  return answer;
	  }
  ...
  public boolean pressLeftPedal(){
    ...
    switch (aStateVehiculeOn){case neutral:
      //setTime(100);
      if (timer>=5){
    	  setStateVehiculeOn(StateVehiculeOn.acceleratebackwards);
    	  exitStateVehiculeOn(); wasEventProcessed = true;break;
    	  } else {
    		  msg += (" since you held it for " + timer + " seconds, the Rover will remain at its current state");
    		  }
      break;
      default: msg+=noChange(stateVehiculeOn);}
    System.out.println(msg);
  ...
  public boolean pressRightPedalOnce(){
    ...
    switch (aStateVehiculeOn){
      ...
      case acceleratebackwards:
    	  //timer=setTime(500);
    	  if (timer >= 5){
    		  setStateVehiculeOn(StateVehiculeOn.constantspeedbackwards);
    		  exitStateVehiculeOn();
    		  wasEventProcessed = true;break;} else {msg += (" since you held it for " + timer + " seconds, the camera will remain at its current state");}break;
      default: msg+=noChange(stateVehiculeOn);
    }
    System.out.println(msg);
    return wasEventProcessed;
